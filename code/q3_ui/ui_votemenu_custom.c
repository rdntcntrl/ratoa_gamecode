...
static void VoteMenu_custom_Event( void* ptr, int event )
{
	switch (((menucommon_s*)ptr)->id)
	{
            ...
            case ID_GO:
                ...
                //trap_Cmd_ExecuteText( EXEC_APPEND, va("cmd callvote custom %s\n",s_votemenu_custom.bEntry[s_votemenu_custom.selection-ID_CUSTOM0].string ) );
                UI_SendClientCommand(va("callvote custom %s\n",s_votemenu_custom.bEntry[s_votemenu_custom.selection-ID_CUSTOM0].string ) );
                UI_PopMenu();
                UI_PopMenu();
                break;
            default:
